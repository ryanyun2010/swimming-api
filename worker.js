        ...
        const responder = async (request, env) => {
            try {
                ...
                if (request.method === "POST" && url.pathname === "/verify") {
                    ...
                    if (!email)
                        return new Response("Unauthorized", { status: 401 });
                    return new Response(JSON.stringify({ allowed: true, email }), {
                        headers: { "Content-Type": "application/json" }
                    });
                }
                if (request.method === "POST" && url.pathname === "/relays") {
                    ...
                    if (!email)
                        return new Response("Unauthorized", { status: 401 });
                    const { time, relay_type, record_1_id, record_2_id, record_3_id, record_4_id } = await request.json();
                    let message = null;
                    ...
                    await env.DB.prepare(`
						...
						VALUES (?, ?, ?, ?, ?, ?)
						`)
                        .bind(time, relay_type, record_1_id, record_2_id, record_3_id, record_4_id)
                        .run();
